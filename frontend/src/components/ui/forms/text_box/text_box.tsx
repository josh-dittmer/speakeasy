...
export default function TextBox({ value, setValue, title, multiline, placeholder, maxChars, optional, hideCount, submitted, setValid }: TextBoxProps) {    
    const ref = createRef<HTMLDivElement>();
    
    const isValid = (value: string): boolean => 
        !!((optional || value.length > 0) && (!maxChars || (maxChars && value.length <= maxChars)));
    ...
    return (
        <div className="text-box">
            ...
            <div className="flex items-center p-1 bg-bg-medium border border-bg-medium rounded" ref={ref}>
                {multiline && (
                    <textarea
                        rows={2}
